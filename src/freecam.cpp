...
void Freecam::ReloadConfig() {
	...
	freeCamera.SetHideHud(config.ReadValue("settings", "hide_hud", 1, ini));
	freeCamera.SetFreezeEntities(config.ReadValue("settings", "freeze_entities", 1, ini));
	freeCamera.SetSmoothCamera(config.ReadValue("settings", "smooth_camera", 1, ini));
